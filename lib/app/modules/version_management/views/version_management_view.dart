...
class VersionManagementView extends StatefulWidget {
  const VersionManagementView({super.key});

  @override
  VersionManagementViewState createState() => VersionManagementViewState();
}

class VersionManagementViewState extends State<VersionManagementView> {
  ...
  Future<void> _loadPageData(int page) async {
    ...
    try {
      final controller = Get.find<VersionManagementController>();
  ...
  // 构建版本卡片
  Widget _buildVersionCard(Document document, int originalIndex,
      VersionManagementController controller) {
  ...
  // 构建发布模式按钮（单个切换）
  Widget _buildPublishModeButton(
      int originalIndex, VersionManagementController controller) {
    // 从当前版本数据重新获取全量模式状态
  ...
  // 构建上线状态按钮（单个切换）
  Widget _buildOnlineStatusButton(int originalIndex,
      VersionManagementController controller, bool isEnable) {
    return _buildSingleToggleButton(
      activeLabel: '已上线',
      inactiveLabel: '未上线',
  ...
  // 构建手机号管理区域
  Widget _buildPhoneManagement(int originalIndex,
      VersionManagementController controller, List<String> allowPhones) {
    // 无条件显示手机号管理区域
  ...
  // 构建手机号管理内容
  Widget _buildPhoneManagementContent(int originalIndex,
      VersionManagementController controller, List<String> allowPhones) {
    return Padding(
  ...
  // 构建手机号列表
  Widget _buildPhoneNumberList(int originalIndex,
      VersionManagementController controller, List<String> allowPhones) {
    return SizedBox(
      width: double.infinity,
      child: Wrap(
  ...
  Future<void> _handleFullModeSwitch(
      int originalIndex, VersionManagementController controller) async {
  ...
  // 处理上线状态切换
  Future<void> _handleOnlineStatusSwitch(int originalIndex,
      VersionManagementController controller, bool isEnable) async {
  ...
  // 构建页码控制面板
  Widget _buildPaginationPanel(VersionManagementController controller) {
    return Obx(() {
      final currentPage = controller.currentPage.value;
      final totalPages = controller.totalPages.value;
  ...
  // 添加手机号
  void _addPhoneNumber(int originalIndex,
      VersionManagementController controller, List<String> allowPhones) {
    if (_phoneController.text.isNotEmpty) {
      ...
      final cleanAllowPhones =
          allowPhones.map((phone) => _cleanPhoneNumber(phone)).toList();
