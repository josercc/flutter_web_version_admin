...
class HotfixView extends StatefulWidget {
  const HotfixView({super.key});

  @override
  HotfixViewState createState() => HotfixViewState();
}

class HotfixViewState extends State<HotfixView> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      // 距离底部200像素时开始加载更多
      final controller = Get.find<HotfixController>();
  ...
  Widget _buildVersionCard(
      Document document, int originalIndex, HotfixController controller) {
  ...
  // 构建卡片操作按钮
  Widget _buildCardActions(int originalIndex, HotfixController controller,
  ...
  Widget _buildPublishModeButton(
      int originalIndex, HotfixController controller) {
    // 从当前版本数据重新获取全量模式状态
  ...
  Widget _buildOnlineStatusButton(
      int originalIndex, HotfixController controller, bool isEnable) {
    return _buildSingleToggleButton(
      activeLabel: '已上线',
      inactiveLabel: '未上线',
  ...
  // 构建手机号管理区域
  Widget _buildPhoneManagement(int originalIndex, HotfixController controller,
  ...
  Widget _buildPhoneManagementContent(int originalIndex,
      HotfixController controller, List<String> allowPhones) {
    return Padding(
  ...
  // 构建手机号列表
  Widget _buildPhoneNumberList(int originalIndex, HotfixController controller,
      List<String> allowPhones) {
    return SizedBox(
      width: double.infinity,
      child: Wrap(
  ...
  Future<void> _handleFullModeSwitch(
      int originalIndex, HotfixController controller) async {
  ...
  Future<void> _handleOnlineStatusSwitch(
      int originalIndex, HotfixController controller, bool isEnable) async {
  ...
  // 构建加载更多指示器
  Widget _buildLoadMoreIndicator(HotfixController controller) {
    return Obx(() {
  ...
  // 添加手机号
  void _addPhoneNumber(int originalIndex, HotfixController controller,
      List<String> allowPhones) {
  ...
  // 删除手机号
  void _removePhoneNumber(int originalIndex, HotfixController controller,
