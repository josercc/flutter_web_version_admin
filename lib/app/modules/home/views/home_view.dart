  ...
  List<Document> _getFilteredVersions(List<Document> versions) {
    return versions.where((document) {
      ...
      if (_routeNameFilter.text.isNotEmpty) {
        if (!routeName
            .toLowerCase()
            .contains(_routeNameFilter.text.toLowerCase())) {
          return false;
        }
      }
      ...
      if (_environmentFilter != '全部') {
        if (_environmentFilter == '测试环境' && (!isEnable || !isTestEnvironment)) {
          return false;
        }
        if (_environmentFilter == '正式环境' && (!isEnable || isTestEnvironment)) {
          return false;
        }
      }
      ...
      if (_phoneNumberFilter.text.isNotEmpty) {
        ...
        bool hasMatchingPhone = allowPhones
            .any((phone) => phone.toLowerCase().contains(phoneFilter));
                                  ...
                                  Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        // 仅在指定手机号模式下显示手机号管理区域
                                        if (!(_isFullPublish[originalIndex] ??
                                            false))
                                          Column(
                                            children: [
                                              const SizedBox(
